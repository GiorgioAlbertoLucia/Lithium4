...
float maxRooDataHist(const RooDataHist* dataHist) {
    float maxVal = 0;
    for (int i = 0; i < dataHist->numEntries(); ++i) {
        const auto& point = dataHist->get(i);
        float yVal = point->getRealValue("y");
        if (yVal > maxVal) {
            maxVal = yVal;
        }
    }
...
bool loadSameMixed(TH1F* &h_same, TH1F* &h_mixed, const char* sign, CentralityConfig centralityConfig);
void computeCorrelationFunction(const TH1F *h_same, const TH1F *h_mixed, TH1F* &h_correlation);
void prepareSignalModel(RooRealVar &kstar, RooDataSet* &signal_data, RooKeysPdf* &signal_pdf, TDirectory *outfile);
void prepareBackgroundModel(RooRealVar &kstar, RooDataSet* &background_data, RooKeysPdf* &background_pdf, TDirectory *outfile, 
                                const char* sign, CentralityConfig centralityConfig);
void prefitBackground(RooRealVar &kstar, TH1F *h_data, RooDataHist* &data, RooAddPdf* &model, RooRealVar& nbkg,
                        TDirectory *outfile, 
...
double fit(RooRealVar &kstar, RooDataHist* &data, RooAddPdf* &model, TDirectory *outfile, 
            double& sig_normalisation_at_reference,
            const char* fit_name = "fit_result", bool do_drawing = false);
double computeYield(RooAbsPdf* model_pdf, RooRealVar& nbkg, RooRealVar& nsig, RooRealVar& xvar, 
...
void saveHighYieldFit(RooRealVar& kstar, RooDataHist* &data, RooAddPdf* &model, int iter, TDirectory* outfile);
void setupOutputDirectories(TDirectory* outfile);
IterationResult processSingleIteration(RooRealVar& kstar, RooAddPdf* model,
                                        RooRealVar& nsig, RooRealVar& nbkg,
                                        TH1F* &h_same_iter, TH1F* &h_mixed_iter, TH1F* &h_correlation_iter,
                                        const TH1F* h_same, const TH1F* h_mixed, TDirectory* outfile, int iter, bool do_drawing,
                                        SamplingMethod sampling_method);
void computeYieldError();
...
bool loadSameMixed(TH1F* &h_same, TH1F* &h_mixed, const char* sign, CentralityConfig centralityConfig) {
    ...
    if (!h_same || !h_mixed) {
        ...
        std::cerr << "Histogram names: " << h_same_name << ", " << h_mixed_name << std::endl;
        file->Close();
        return false;
    }
...
void prepareCorrelationFunction(const TH1F *h_same, const TH1F *h_mixed, TH1F* &h_correlation, 
                                   const char* sign, CentralityConfig centralityConfig) {
    if (strcmp(Config::CORRELATION_OPTION, "load") == 0) {
        ...
        //std::string histName = std::string(sign) + "/hCorrelation" + centrality + "Syst";
        h_correlation = (TH1F*)file->Get(histName.c_str());
...
void computeCorrelationFunction(const TH1F *h_same, const TH1F *h_mixed, TH1F* &h_correlation) {
    ...
    for (int ibin = 1; ibin <= nbins + 1; ++ibin) {
        ...
        if (mixed_val <= 0 || same_val <= 0) {
            h_correlation->SetBinContent(ibin, 1e-12);
            h_correlation->SetBinError(ibin, 1e12);
            continue;
        }
...
//void prepareSignalModel(RooRealVar &kstar, RooDataHist* &signal_data, RooHistPdf* &signal_pdf, TFile *outfile) {
...
//void prepareBackgroundModel(RooRealVar &kstar, RooDataHist* &background_data, RooHistPdf* &background_pdf, TFile *outfile, 
//                                const char* sign, CentralityConfig centralityConfig) {
//    using namespace RooFit;
//
...
void prepareBackgroundModel(RooRealVar &kstar, RooDataSet* &background_data, RooKeysPdf* &background_pdf, TDirectory *outfile, 
    ...
    auto h_correlation_background = (TH1F*)file->Get(Form("%s/%s/hLambdaSigmaCorrectedCk_Smeared", sign, centralityConfig.name));
    if (!h_correlation_background) {
        std::cerr << "Error: Cannot load background histogram" << std::endl;
        file->Close();
        return;
...
void prefitBackground(RooRealVar &kstar, TH1F* h_data, RooDataHist* &data, RooAddPdf* &model, RooRealVar& nbkg, TDirectory *outfile, 
                       double& bkg_normalisation_at_reference, double& bkg_full_correction,
    ...
    TCanvas canvas("canvas_prefitBackground", "Prefit Background", 800, 600);
    frame->Draw();
...
double fit(RooRealVar &kstar, RooDataHist* &data, RooAddPdf* &model,
    ...
    frame->SetMaximum(1.5 * maxRooDataHist(data));
    data->plotOn(frame, MarkerStyle(20), MarkerSize(0.5));
...
double computeYield(RooAbsPdf* model_pdf, RooRealVar& nsig, RooRealVar& nbkg, RooRealVar& xvar, 
    ...
    nbkg.setVal(0.);
    const double sig_value_at_reference = model_pdf->getVal(norm_set);
    ...
    for (int ibin = 1; ibin <= nbins; ++ibin) {
        ...
        nsig.setVal(nsig_stored);
        nbkg.setVal(0.);
        const double signal_value = model_pdf->getVal(norm_set) * sig_correction;
...
void setupOutputDirectories(TDirectory* outfile) {
    outfile->mkdir("prefitBackground");
    outfile->mkdir("fit_results");
    outfile->mkdir("yield_results");
    outfile->mkdir("high_yield_results");
}

IterationResult processSingleIteration(RooRealVar& kstar, RooAddPdf* model, RooRealVar& nsig, RooRealVar& nbkg,
                                            TH1F* &h_same_iter, TH1F* &h_mixed_iter, TH1F* &h_correlation_iter,
                                            const TH1F* h_same, const TH1F* h_mixed, const TH1F* h_correlation,
                                            TDirectory* outfile, int iter, bool do_drawing,
                                            SamplingMethod sampling_method) {
    ...
    prefitBackground(kstar, h_correlation_iter, data_prefit, model, nbkg,
    ...
    if (yield > Config::HIGH_YIELD_THRESHOLD) {
        saveHighYieldFit(kstar, data_iter, model, iter, outfile);
    }
...
void computeYieldAndUpperLimit() {

    // Silence RooFit output
    ...
    for (const auto& cent : kCentralities) {
        for (const auto& sign : {"Antimatter", "Matter", "Both"}) {
            ...
            TH1F *h_same, *h_mixed;
            if (!loadSameMixed(h_same, h_mixed, sign, cent)) {
                std::cerr << "Failed to load histograms for " << sign << " in centrality " << cent.name << ". Exiting." << std::endl;
                continue;
            }
            ...
            prepareCorrelationFunction(h_same, h_mixed, h_correlation, sign, cent);
            ...
            prepareSignalModel(kstar, signal_data, signal_pdf, outdir);
            ...
            prepareBackgroundModel(kstar, background_data, background_pdf, outdir,
                                    sign, cent);
            ...
            std::cout << "Analysis complete. Results saved to " << Config::OUTPUT_FILE << std::endl;
        }
    }
